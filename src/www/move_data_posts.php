...
// get all products
if($query->sql("SELECT * FROM $from_db.`itemtype_blog`")) {
	$results = $query->results();
	foreach($results as $result) {
//		print_r($result);

		print "# " . $result["item_id"] ." - " . $result["name"] . " - " . $result["html"] . " - " . $result["timestamp"] . "<br>";
...
//		$files = $result["files"];
		...
		if($run_as_test) {
			...
			// ITEM
			// create item
			if($query->sql("INSERT INTO $to_db.`items` VALUES(DEFAULT, DEFAULT, 1, 'post', DEFAULT, '".$result["timestamp"]."', CURRENT_TIMESTAMP, '".$result["timestamp"]."')")) {
				print "Item created<br>";

				$item_id = $query->lastInsertId();

				// create wish
				$sql = "INSERT INTO $to_db.`item_post` VALUES(DEFAULT, $item_id, '$name', '$html')";
				print $sql;
				if($query->sql($sql)) {
					print "Post created<br>";
					...
					print "Post failed ($item_id, $name)<br>";
